...
impl Step {
    pub fn new(pair: crate::Pair, context: &mut Context) -> Option<Self> {
        ...
        let step = match pair.as_rule() {
            ...
            Rule::relation => {
                ...
                let lhs = Term::new(pairs.next().unwrap(), context)?;
                let operator = pairs.next().unwrap().to_string();
    ...
    pub fn handles_mut<'a>(&'a mut self) -> Box<dyn Iterator<Item = &mut Handle> + 'a> {
        match self {
            ...
            Self::Relation(lhs, _, rhs) => Box::new(lhs.handles_mut().chain(rhs.handles_mut())),
    ...
    pub fn identifiers<'a>(&'a self) -> Box<dyn Iterator<Item = Identifier> + 'a> {
        match self {
            ...
            Self::Relation(lhs, _, rhs) => Box::new(lhs.identifiers().chain(rhs.identifiers())),
...
impl Display for Step {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        match self {
            ...
            Self::Relation(lhs, operator, rhs) => write!(f, "{} {} {}", lhs, operator, rhs),
