...
pub struct NativeFunction<'p> {
    function: Rc<Box<dyn Fn(Vec<Option<Value>>) -> Vec<Vec<Option<Value>>> + 'p>>,
}

impl<'p> NativeFunction<'p> {
    pub(crate) fn new<F>(function: F) -> Self
    where
        F: Fn(Vec<Option<Value>>) -> Vec<Vec<Option<Value>>> + 'p,
    {
        Self {
    ...
    pub(crate) fn call(&self, values: Vec<Option<Value>>) -> Vec<Vec<Option<Value>>> {
